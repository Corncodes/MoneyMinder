## August 23rd 2023
Franz drove. UD in CRUD for expenses/expense items completed.


## August 22nd 2023
Amanuel drove. CR in CRUD for expenses/expense items completed. Need to complete UD in CRUD and tidy up the R.


## August 21st 2023
Will drove. Completed the budget endpoints.


## August 17th 2023
Franz drove! Then Corn drove. I started creating the router, query, and models for our budgets until we were rudely interrupted by power outages on my end.


## August 16th 2023
Amanuel drove. Finished back-end authentication.


## August 15th 2023
Will drove. Started on back-end authentication. Created models, queries, and routers directories for accounts.py. Built accounts table in moneyminder.sql file.


## August 14th 2023
Corn drove. Completed issue # 1 aka creating a database.
